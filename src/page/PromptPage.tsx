...
export default function PromptPage() {
  ...
  const getResult = async () => {
    ...
    const prompt = `위치는 ${address}이고, ${target}를 찾아줘.
      내 요구사항은 다음과 같아: 현재 ${address}에서 가까우며, ${textArea}.
      이 요구사항에 맞게 ${target}를 추천해줘.
      결과는 첫째줄에 ${target}이름, 둘째줄에 ${target}주소, 셋째줄에 선택한 이유 알려줘. 다른 부가적인 말 절대 추가하지 말고 딱 이름, 주소, 이유만 적어. 
      이름은 다른 지역에 같은 이름이 있을 수도 있으니까 그 부분 신경써서 결과가 하나만 나올 수 있도록 해줘. 
      내 모든 요청사항에 대해서 google지도 말고 네이버 지도에서 검색하도록 해. 부합하는 곳이 없다면 꼭 가짜 정보를 만들지 말고 "가게 정보를 찾을 수 없습니다"란 메시지를 3줄에 걸쳐 답변하도록 해. `;
